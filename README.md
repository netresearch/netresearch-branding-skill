...
git clone https://github.com/netresearch/netresearch-branding-skill.git \
  ~/.claude/skills/netresearch-branding
```

It loads as `netresearch-branding@skills-dir` on the next session. Update with `git -C ~/.claude/skills/netresearch-branding pull` and start a new session; remove it by deleting the directory. This route has no `claude plugin update`.
